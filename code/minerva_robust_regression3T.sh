...
.libPaths(c("/hpc/users/heissj01/.Rlib",.libPaths()))
...
load("transfer_phth3T.Rdata")

h = plan(batchtools_lsf,template="batchtools.lsf.tmpl",resources=
list(queue="premium",walltime=3600*2,memory=300,allocation="acc_envepi"))
...
saveRDS(pvals,file="pvals_phth3T.rds")
